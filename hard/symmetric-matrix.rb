...
["1","0","1","<>","0","1","0","<>","1","0","1"]. 
There are two "<>", so 2 + 1 = 3. Therefore there will be three rows in the 
...
and then determine whether the matrix is symmetric, in other words, if matrix 
M is equal to M transpose. If it is, return the string symmetric and if it isn't 
...
def SymmetricMatrix(strArr)
  return 'not possible' if strArr.index('<>') != strArr.count('<>') + 1
  matrix = strArr.join(' ').split('<>').map { |s| s.split(' ') }
  return matrix == matrix.transpose ? 'symmetric' : 'not symmetric'
end
   
# keep this function call here 
# to see how to enter arguments in Ruby scroll down   
# SymmetricMatrix(STDIN.gets)
p SymmetricMatrix(["1","0","1","<>","0","1","0","<>","1","0","1"]) \
== 'symmetric'
p SymmetricMatrix(["5","0","<>","0","5"]) \
== 'symmetric'
p SymmetricMatrix(["1","2","4","<>","2","1","1","<>","-4","1","-1"]) \
== 'not symmetric'